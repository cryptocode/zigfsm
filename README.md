<img align="right" width="160" height="160" src="https://user-images.githubusercontent.com/34946442/152222895-9c8adb22-a22d-4bce-a513-3486ca28bdd5.png"> zig**fsm** is a [finite state machine](https://en.wikipedia.org/wiki/Finite-state_machine) library for Zig. This library tracks [Zig master](https://github.com/ziglang/zig).
...
const FSM = fsm.StateMachine(State, Trigger, .off);
```
...
### Defining transitions and triggers as a table

Rather than calling addTransition and addTrigger, `StateMachineFromTable` can be used to pass a table of triggers and state transitions.
...
In many non-trivial applications, especially when using triggers, you might want to know when a transition happens. Or you might need to cancel a transition in specific situations.
